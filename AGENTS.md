...
11. **TLP power tuning — per-profile.** Power profile switching uses TLP (via `~/.local/bin/power-profile`, bound to `$mod+Ctrl+p`), controlled without root through `tlpctl` (the `tlp-pd` D-Bus client) — not `tlp` (needs root). TLP has 3 profiles, each with its own parameter set (suffix `_AC`/`_BAT`/`_SAV` maps to performance/balanced/power-saver respectively). The per-profile config lives in `/etc/tlp.d/profiles.conf` (system file, not in this repo — recreate on reinstall), which declares the default profile per power source:
   - **Pluggled in (AC)** → `performance` profile (full speed, uncapped: `CPU_BOOST_ON_AC=1`, `CPU_SCALING_MAX_FREQ_ON_AC=0`, `governor=performance`) — set via `TLP_PROFILE_AC=PRF`.
   - **On battery** → `power-saver` profile (aggressive 1 GHz cap: `CPU_BOOST_ON_SAV=0`, `CPU_SCALING_MAX_FREQ_ON_SAV=1000000`, `governor=powersave`, plus `PCIE_ASPM` powersupersave) — set via `TLP_PROFILE_BAT=SAV`. This keeps all-core load power flat (~8 W idle floor dominated by screen/WiFi/SSD).
   - **balanced** profile is deliberately left at TLP's light defaults (a middle option).
   After editing, apply with `sudo tlp start`. **Caveat:** switching profiles can leave a stale `scaling_max_freq` (e.g. stuck at 400000 kHz) in amd-pstate-epp that isn't cleared by TLP; if the CPU seems capped after a profile switch, release it with `echo 4785000 > /sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq` (or reboot). This machine is normally in **Integrated** GPU mode (supergfxctl) so the NVIDIA dGPU is fully off; don't re-enable unless needed.
