...
// protoDocOutputName strips common proto source prefixes from the file name
// to produce a clean relative path for the markdown output. It removes the
// proto/ prefix if present, so that proto/dotfilesd/v1/foo.proto becomes
// dotfilesd/v1/foo.md.
func protoDocOutputName(protoName string) string {
	const prefix = "proto/"
	name := strings.TrimSuffix(protoName, ".proto") + ".md"
